...
var data = new Uint8Array(RESX * RESY * 3);
var dataPtr = 0;
    ...
    for (let x = 0; x < resX; x++) {
      ...
      for (let itr = maxItr; itr > 0; itr--) {
        ...
        if (zs > 1073741824) {
          ...
          data[dataPtr] = Math.floor(0.9 * (127 - 127 * Math.cos(2 * Math.PI * (0.45 * smooth + 0.5)))); dataPtr++;
          data[dataPtr] = Math.floor(127 - 127 * Math.cos(2 * Math.PI * (0.45 * smooth + 0.9))); dataPtr++;
          data[dataPtr] = Math.floor(127 - 127 * Math.cos(2 * Math.PI * (0.45 * smooth + 0.1))); dataPtr++;
          itr = 0;
        } else if (zs < low) {
          low = zs;
        }
      ...
      if (zs <= 1073741824) {
        ...
        data[dataPtr] = 0; dataPtr++;
        data[dataPtr] = Math.floor(127 - 127 * Math.cos(Math.log(low) / 2.0)); dataPtr++;
      }
